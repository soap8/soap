...
namespace OnlineShoppingStore.Repository
{
    public class GenericRepository<Tbl_Entity> : IRepository<Tbl_Entity> where Tbl_Entity : class
    {
        ...
        private Entities2 _DBEntity;
        public GenericRepository(Entities2 DBEntity)
        {
            _DBEntity = DBEntity;
        ...
        public IEnumerable<Tbl_Entity> GetRecordsToShow(int PageNo, int PageSize, int CurrentPage, Expression<Func<Tbl_Entity, bool>> wherePredicr, Expression<Func<Tbl_Entity, int>> orderByPredict)
        {
            if(wherePredicr != null)
            {
                return _dbSet.OrderBy(orderByPredict).Where(wherePredicr).ToList();
            }
            else
            {
